...
public class Explosion extends Entity {
    ...
    private void chooseSprite() {
        switch (type) {
            case UP:
                if (!powerUp || map[yUnit - 1][xUnit] == '#') {
                    sprite = Sprite.movingSprite(Sprite.explosion_vertical_top_last, Sprite.explosion_vertical_top_last1,
                            Sprite.explosion_vertical_top_last2, animate, t);
                } else {
                    sprite = Sprite.movingSprite(Sprite.explosion_vertical, Sprite.explosion_vertical1,
                            Sprite.explosion_vertical2, animate, t);
                }
                break;
            case RIGHT:
            ...
            case LEFT:
                if (!powerUp || map[yUnit][xUnit - 1] == '#') {
                    sprite = Sprite.movingSprite(Sprite.explosion_horizontal_left_last, Sprite.explosion_horizontal_left_last1,
                            Sprite.explosion_horizontal_left_last2, animate, t);
                } else {
                    sprite = Sprite.movingSprite(Sprite.explosion_horizontal, Sprite.explosion_horizontal1,
                            Sprite.explosion_horizontal2, animate, t);
                }
            ...
            case LEFT1:
                ...
                break;
            case CENTER:
                sprite = Sprite.movingSprite(Sprite.bomb_exploded, Sprite.bomb_exploded1, Sprite.bomb_exploded2, animate, t);
                break;
        }
    }
    ...
    public boolean isExplosion() {
        switch (type) {
            case UP1:
                if (map[yUnit + 1][xUnit] != '#' && map[yUnit][xUnit] != '#' && powerUp) {
                    return true;
                }
                break;
            case RIGHT1:
                if (map[yUnit][xUnit - 1] != '#' && map[yUnit][xUnit] != '#' && powerUp) {
                    return true;
                }
                break;
            case DOWN1:
